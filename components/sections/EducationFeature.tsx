...
/**
 * A single workshop, set as a filed record.
 *
 * The featured workshop has already taken place. It is labelled PAST WORKSHOP
 * and carries no registration, ticket link or capacity — and no upcoming event
 * is fabricated to fill the space.
 *
 * `headingLevel` keeps the document outline intact wherever the record sits:
 * h3 under a section heading, h2 when it stands alone.
 */
export function WorkshopRecord({
  ...
  const EventTitle = headingLevel;
  const TopicsTitle = headingLevel === "h2" ? "h3" : "h4";
  ...
  return (
    <article className="border border-line">
      {/* Status and date lead, as they would on a filed archive entry. */}
      <Reveal>
        <div className="flex flex-wrap items-center justify-between gap-x-6 gap-y-3 border-b border-line bg-ink px-5 py-4 text-white md:px-8">
          <p className="ke-label flex items-center gap-2.5">
            ...
            {event.status === "past" ? "Past Workshop" : "Upcoming Workshop"}
          </p>
          <p className="flex items-center gap-2.5 font-mono text-sm tabular-nums tracking-tight text-white md:text-base">
            <Calendar className="h-4 w-4 text-accent-on-dark" aria-hidden="true" />
      ...
      <div className="grid gap-px bg-line lg:grid-cols-2">
        ...
        <Reveal className="flex flex-col bg-paper p-7 md:p-10 lg:p-12">
          <EventTitle className="ke-h3 max-w-[20ch] text-ink">{event.title}</EventTitle>
          <p className="mt-3 font-display text-lg font-semibold tracking-[-0.02em] text-accent-ink">
            {event.subtitle}
          </p>

          {event.partner ? (
            <p className="mt-5 inline-flex items-center gap-2 self-start border border-line px-3 py-2 text-[0.8125rem] text-steel">
          ...
          <dl className="mt-8 grid gap-x-8 gap-y-5 border-t border-line pt-7 sm:grid-cols-3">
            {details.map((detail) => (
              <div key={detail.label} className="flex gap-3">
                ...
                />
                <div>
                  <dt className="ke-label mb-1.5 text-steel">{detail.label}</dt>
                  <dd className="ke-body-sm leading-relaxed text-ink">{detail.value}</dd>
                </div>
              </div>
            ))}
          </dl>
          ...
          <div className="mt-8 border-t border-line pt-7">
            <TopicsTitle className="ke-label mb-4 text-steel">Topics covered</TopicsTitle>
